...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        self.feature_buffer = deque(maxlen=self.buffer_limit)
        self.sample_buffer = []
        self.residuals = deque(maxlen=self.residual_window)
    ...
    def test(self, setting):
        ...
        # cache dataset for potential incremental fine-tuning during drift handling
        self.latest_dataset = test_data
    ...
    def _incremental_update(self):
        if not self.sample_buffer or self.latest_dataset is None:
            return
        criterion = self._select_criterion()
        optimizer = optim.Adam(self.model.parameters(), lr=self.ft_lr)
        self.model.train()
        for _ in range(self.ft_epochs):
            for batch_x, batch_y, batch_x_mark, batch_y_mark in self.sample_buffer:
                pred, true = self._process_one_batch(
                    self.latest_dataset,
                    batch_x.clone(), batch_y.clone(), batch_x_mark.clone(), batch_y_mark.clone(),
                )
                loss = criterion(pred, true)
                optimizer.zero_grad()
                loss.backward()
    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        if self.sleep_interval > 1  or self.args.online_adjust > 0:
            ...
            residual = (rearrange(batch_y, 'b t d -> b (t d)') - outputs.detach()).view(outputs.size(0), -1)
            self.residuals.append(residual)
            self.sample_buffer.append((batch_x.detach(), batch_y.detach(), batch_x_mark.detach(), batch_y_mark.detach()))
            ...
            if real_drift:
                print(f"[New D3A] Real drift detected (mu={mu:.4f}, sigma={sigma:.4f}); incremental update.")
                self._incremental_update()
                self.residuals.clear()
            elif virtual_drift:
                print(f"[New D3A] Virtual drift warning (energy={energy_score:.4f}).")
